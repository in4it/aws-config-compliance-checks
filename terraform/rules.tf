...
resource "aws_config_config_rule" "s3-vpc-traffic-only" {
  count = var.rule_s3_vpc_traffic_only_enabled ? 1 : 0
  description = "Checks if S3 has a vpc traffic only configuration is set for s3 buckets"
  input_parameters = jsonencode(
    {
      excludeBuckets = var.exclude_buckets
    }
  )
  name = "${var.resource_name_prefix}-s3-vpc-traffic-only"
  ...
  source {
    owner             = "CUSTOM_LAMBDA"
    source_identifier = aws_lambda_function.s3-vpc-traffic-only.arn
